#include <opencv2/opencv.hpp> // opencv2/opencv.hpp：引入OpenCV库，用于图像处理。
#include <string> // string：支持字符串操作。
#include <nmmintrin.h> // nmmintrin.h：用于高级CPU指令优化。
#include <chrono> // chrono：用于计时操作。
...
typedef vector<uint32_t> DescType; // Descriptor type：定义为存储描述子的向量，每个描述子由多个uint32_t组成。
